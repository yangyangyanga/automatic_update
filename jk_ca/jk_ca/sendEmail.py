...
def SendEmailMain():
    ...
    sql = "select old_id,url_old,university,change_context from Label_content where old_id like 'c%' and change_context like '%1%' order by university"
    cursor.execute(sql)
    result = cursor.fetchall()
    conn.commit()
    sql2 = "select count(*),university from Label_content where change_context like '%1%' and old_id like 'c%' GROUP BY university"
    cursor.execute(sql2)
    conn.commit()
    ...
    if messagetext!='\n':
        sendEmail(subject='加拿大变化邮件',messageText=messagetext)
# SendEmailMain()
