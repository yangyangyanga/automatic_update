...
from jk_en.items import JkEnItem,get_item
from decimal import *
from jk_en.middlewares import remove_class,clearallSB,runSelenuim
class SchoolJkSpiderSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for sl,il in zip(school_list,id_lists):
            ...
            for r in results:
                ...
                print(cout)
                with open('Englandfileout.txt', 'a+', encoding='utf-8') as fi:
                    fi.write(cout + '\n')
    ...
    def get_school_id(self):
        ...
        cursor = conn.cursor()
        sql = "select ename,id from school_info where country_id=2"
        cursor.execute(sql)
        conn.commit()
    ...
    def get_url_id(self,school):
        ...
        conn.ping(reconnect=True)
        cursor = conn.cursor()
        selu = 'select url,id,degree_name,programme_en from tmp_school_uk_ben where university="%s"' % school
        # 取出本科数据
        cursor.execute(selu)
        resultu = cursor.fetchall()
        resultu = list(map(lambda x: [x[0], 'eu' + str(x[1]),x[2],x[3]], resultu))
        # print(resultu)
        selp = 'select url,id,degree_name,programme_en from tmp_school_uk_yan where university="%s"' % school
        # 取出研究生数据
        cursor.execute(selp)
        resultp = cursor.fetchall()
        resultp = list(map(lambda x: [x[0], 'ep' + str(x[1]),x[2],x[3]], resultp))
        # conn.close()
        # print(resultp)
