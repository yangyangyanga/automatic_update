...
class JkEnPipeline(object):
    ...
    def judgeMS(self,item):
        sql = "select old_id from Label_content WHERE old_id = '%s'" % item['old_id']
    ...
    def insertMysql(self,item):
        ...
        date = item
        # 表未定，暂时省略部分内容
        sql = "insert into Label_content(old_id,major_name,degree_name,web_label,web_content,state_code,url_now,url_old,update_time,university) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) " \
    ...
    def compareMysql(self,item):
        self.conn.ping(reconnect=True)
        sql = "select web_label,web_content from Label_content where old_id='%s'" % item['old_id']
        self.cursor.execute(sql)
        self.conn.commit()
        ...
        sqls = "update Label_content set update_time=%s,change_context=%s,web_label=%s,web_content=%s where old_id=%s"
