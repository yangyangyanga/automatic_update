...
from jk_au.items import JkAuItem,get_item
from decimal import *
from jk_au.middlewares import remove_class,clearallSB,runSelenuim
class SchoolJkSpiderSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for sl,il in zip(school_list,id_lists):
            ...
            for r in results:
                ...
                print(cout)
                with open('Australiafileout.txt', 'a+', encoding='utf-8') as fi:
                    fi.write(cout + '\n')
                yield scrapy.Request(url=r[0],callback=self.parse_main,meta={'url':r[0],'sid':str(r[1]),'degree_name':r[2],'major_name_en':r[3],'school_name':sl})
    def parse_main(self,response):
        item=get_item(JkAuItem)
        # print(item)
    ...
    def get_school_id(self):
        ...
        cursor = conn.cursor()
        sql = "select ename,id from school_info where country_id=3"
        cursor.execute(sql)
        conn.commit()
    ...
    def get_url_id(self,school):
        ...
        conn.ping(reconnect=True)
        cursor = conn.cursor()
        selu = 'select url,id,degree_name_en,programme_en from tmp_school_au_ben where university="%s"' % school
        # 取出本科数据
        cursor.execute(selu)
        resultu = cursor.fetchall()
        resultu = list(map(lambda x: [x[0], 'au' + str(x[1]),x[2],x[3]], resultu))
        # print(resultu)
        selp = 'select url,id,degree_name_en,programme_en from tmp_school_au_yan where university="%s"' % school
        # 取出研究生数据
        cursor.execute(selp)
        resultp = cursor.fetchall()
        resultp = list(map(lambda x: [x[0], 'ap' + str(x[1]),x[2],x[3]], resultp))
        # conn.close()
        # print(resultp)
